...
# The release tag and the APK filename infix do not always match, e.g. tag
# "v0.3b" ships "wa2_0.3cn.apk". Hence two separate placeholders.
APK_URL = "https://github.com/dorakyuraduang/wa2-godot/releases/download/{tag}/wa2_{name}cn.apk"
...
def fetch_apk(tag, dest, local=None, apk_name=None):
    if local:
        log(f"using local APK: {local}")
        shutil.copyfile(local, dest)
        return
    url = APK_URL.format(tag=tag, name=apk_name or tag)
...
def main():
    ...
    ap.add_argument("--apk-name", help="filename infix in wa2_<name>cn.apk (defaults to --tag)")
    ...
    try:
        ...
        fetch_apk(args.tag, apk, args.apk, args.apk_name)
